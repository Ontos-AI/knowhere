...
| `grep` | Exact string / regex / identifier / number lookup that must run against body text | scoped by document/section/chunk_type | Call args are `pattern` (one term) and/or `patterns` (a list of terms OR'd together) — pass every candidate term/synonym you want to check in one call's `patterns` list rather than issuing several parallel `corpus.grep` calls in the same turn. Returns match count plus snippets, so ANY/ALL logic can also close over body text, not just titles. Each hit includes `document_id`. |
