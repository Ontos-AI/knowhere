...
| `grep` | Exact string / regex / identifier / number lookup that must run against body text | scoped by document/section/chunk_type | Call args are `pattern` (one term) and/or `patterns` (a list of terms OR'd together) — pass every candidate term/synonym you want to check in one call's `patterns` list rather than issuing several parallel `corpus.grep` calls in the same turn. Returns match count plus snippets. Each hit includes `chunk_type` and `document_id`. Body hits (`text`/`page`) also include `section_path` for `read`. `image`/`table` hits also include `chunk_id` — their `section_path` is the document Root, not the host section; `read` those with `chunk_id`, or use `assets` `host_of` to find the host. Never pass `file_path` as `chunk_id`. |
| `recall` | A fuzzy question where you don't know where the answer lives | namespace or scoped | Ranked candidates from `path_content` (BM25) + `term` (substring) channels fused by RRF; `vector` is reserved — see §5. Each hit includes `chunk_type` and `document_id`. Body hits include `section_path` for `read`. `image`/`table` hits include `chunk_id` — their `section_path` is the document Root, not the host section; `read` those with `chunk_id`, or use `assets` `host_of`. |
