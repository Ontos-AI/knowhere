...
async def test_content_channel_uses_bounded_or_fts_after_scope_filters(
    ...
    rows = await channels.content_channel(
        ...
        exclude_sections=[{"document_id": "doc-1", "section_path": "Root / Hidden"}],
...
@pytest.mark.asyncio
async def test_bm25_channel_uses_full_scan_fallback_and_logs_metrics(
    monkeypatch: MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        channels.settings,
    ...
    fallback_sql, _fallback_params = db.calls[1]
    assert "CAST(:fts_tokens AS text[])" in fts_sql
    assert "CAST(:fts_tokens AS text[])" not in fallback_sql
    assert "LIMIT :fts_candidate_limit" not in fallback_sql
    assert fts_params["fts_candidate_limit"] == 5
    assert len(fake_logger.messages) == 1
    assert "content_search_text" in fake_logger.messages[0]
    assert "candidates=1" in fake_logger.messages[0]
    assert "limit=5" in fake_logger.messages[0]
    assert "fallback=True" in fake_logger.messages[0]
