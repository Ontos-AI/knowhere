...
def test_generate_presigned_put_url_includes_oss_callback_params(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        oss_adapter,
    ...
    assert bucket.captured_params is not None
    assert "callback" in bucket.captured_params

    callback_config = _decode_callback_config(bucket.captured_params["callback"])
    assert (
